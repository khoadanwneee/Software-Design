...
describe("notification processor", () => {
  ...
  it("still sends registration confirmation email when email preference is disabled", async () => {
    const prisma = createMemoryPrisma();
  ...
  it("marks FAILED_PROVIDER and skips duplicate terminal re-processing", async () => {
    const prisma = createMemoryPrisma();
    const email = new RecordingAdapter(NotificationChannel.EMAIL, {
      success: false,
      error: "SMTP timeout",
    ...
    await expect(processNotification(makeJob(data, 1, 2), deps)).resolves.toBeUndefined();
    expect(email.calls).toHaveLength(1);
    expect(prisma.state.notifications[0].status).toBe(NotificationStatus.FAILED_PROVIDER);
  ...
  it("still creates in-app registration confirmation when in-app preference is disabled", async () => {
    const prisma = createMemoryPrisma();
