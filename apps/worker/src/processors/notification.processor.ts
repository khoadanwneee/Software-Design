...
export async function processNotification(
  ...
  const enabledChannels = registeredChannels.filter((channel) => deps.channelRegistry.isEnabled(channel, prefs));
  ...
  for (const channel of registeredChannels) {
    if (!deps.channelRegistry.isEnabled(channel, prefs)) {
  ...
  if (hasProviderFailure) {
    const retryableStatus = isFinalAttempt(job)
      ? sentCount > 0
        ? NotificationStatus.PARTIAL_FAILED
        : NotificationStatus.FAILED_PROVIDER
    ...
    deps.metrics.increment(isFinalAttempt(job) ? "failed" : "retried", {
      eventType: data.eventType,
      dedupeKey: data.dedupeKey,
      recipientId: data.userId,
      retryAttempt: attempt,
